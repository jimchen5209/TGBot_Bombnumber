...
class Player(telepot.aio.helper.ChatHandler):
        ...
        if chat_type == 'private':
            if content_type == 'text':
                if initial_msg['text'] == '/start_game':
                    clog('[Info] '+fnick+'('+fuserid+ ') has started a game with the bomb '+ str(self._answer))
            ...
            if content_type == 'text':
                if initial_msg['text'] == '/start_game' or initial_msg['text'] == '/start_game@'+username:
                    clog('[Info] '+fnick+'('+fuserid+ ') in '+ initial_msg['chat']['title']+' ('+str(chat_id)+ ') has started a group game with the bomb '+ str(self._answer))
                    dre = await self.sender.sendMessage('遊戲開始！請猜一個範圍內的數字',reply_to_message_id= initial_msg['message_id'])
                    log("[Debug] Raw sent data:"+str(dre))
                    dre = await self.sender.sendMessage('當猜中數字時就會爆炸，你只有 '+str(Timeout)+' 秒')
                    log("[Debug] Raw sent data:"+str(dre))
                    markup = ForceReply()
                    dre = await self.sender.sendMessage(str(self._cmin)+' - '+str(self._cmax), reply_markup=markup)
                    log("[Debug] Raw sent data:"+str(dre))
                    return True
                else:
                    self.close()
    ...
    async def on_chat_message(self, msg):
        ...
        if chat_type == 'private':
            ...
            if guess != self._answer:
                ...
                dre = await self.sender.sendDocument('http://i.imgur.com/vjrcTIy.gif', caption='Boom!')
                log("[Debug] Raw sent data:"+str(dre))
                dre = await self.sender.sendMessage('遊戲結束，您引爆了炸彈')
                log("[Debug] Raw sent data:"+str(dre))
                dre = await self.sender.sendMessage('/start_game')
                log("[Debug] Raw sent data:"+str(dre))
                clog('[Info] '+fnick+' ('+fuserid+ ') has gotten the bomb and exploded.\n[Info] Game ended.')
                self.close()
        elif chat_type == 'group' or chat_type == 'supergroup':
            try:
                reply_to = msg['reply_to_message']['from']['id']
                ...
                if content_type != 'text':
                    ...
                    if msg['text'] == '/stop' or msg['text'] == '/stop@'+username:
                        ...
                        log("[Debug] Raw sent data:"+str(dre))
                        clog('[Info] '+fnick+' ('+fuserid+ ') in '+msg['chat']['title']+' ('+str(chat_id)+ ') has stopped the game.\n[Info] Game ended.')
                        self.close()
                return
            else:
                if reply_to == bot_me['id']:
                    if content_type != 'text':
                        if content_type == 'left_chat_member':
                            ...
                            markup = ForceReply()
                            dre = await self.sender.sendMessage('麻煩給我一個阿拉伯數字.',reply_to_message_id= msg['message_id'], reply_markup=markup)
                            log("[Debug] Raw sent data:"+str(dre))
                        return
                    if msg['text'] == '/stop' or msg['text'] == '/stop@'+username:
                        dre = await self.sender.sendMessage('遊戲結束,炸彈是 %d' % self._answer,reply_to_message_id= msg['message_id'])
                    ...
                    if guess != self._answer:
                        ...
                        dre = await self.sender.sendMessage('/start_game', reply_markup=None)
                        log("[Debug] Raw sent data:"+str(dre))
                        clog('[Info] '+fnick+' ('+fuserid+ ') in '+msg['chat']['title']+' ('+str(chat_id)+ ') has gotten the bomb and exploded.\n[Info] Game ended.')
                        self.close()
                else:
                    if content_type == 'text':
                        if msg['text'] == '/stop' or msg['text'] == '/stop@'+username:
    ...
    async def on__idle(self, event):
        ...
        dre = await self.sender.sendMessage('/start_game', reply_markup=None)
        clog('[Info] The game of '+str(event['_idle']['source']['id'])+' has expired.\n[Info] Game ended.')
        self.close()
